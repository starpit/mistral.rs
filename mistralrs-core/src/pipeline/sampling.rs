                        ...
                        if block.is_plus {
                            if let Some(hash) = block.content_hash {
                                let block_cache: Vec<Option<crate::pipeline::KvCache>> = seq
                                    ...
                                    .collect();
                                // Store raw (un-RoPE'd) K. With sequential positions,
                                // cached blocks need RoPE at their new positions on
                                // reuse, so pre-RoPE'd K is not useful.
                                prefix_cacher.add_pic_block(hash, block_cache, None);
                            }
                        }
                    }
                }
