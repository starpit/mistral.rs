    ...
    fn forward(
        ...
        if let Some(pic_ctx) = pic_context {
            ...
            let mut y = pic::pic_sdpa_attention(
                &q, &k, &v, pic_ctx, &*self.rotary_emb, kv_cache,
                attention_mask, flash_params, &self.sdpa_params,
            )?;
            if let Some(t) = self.q_proj.quantized_act_type() {
                y = y.to_dtype(t)?;
            }
    ...
    pub fn forward_embeds(
        ...
        let attention_mask = if let Some(pic_ctx) = pic_context {
            ...
            if tgt_len == 1 {
                ...
                let pic_mask = pic_ctx.make_pic_mask(tgt_len, past_kv_len, xs.device(), xs.dtype())?;
...
impl NormalModel for Model {
    ...
    fn pic_pre_rope_k(
        ...
        Ok(Some(self.layers[0].self_attn.rotary_emb.pic_forward_k_only(k, &positions)?))
    }
    fn forward_pic(
        &self,
        input_ids: &Tensor,
